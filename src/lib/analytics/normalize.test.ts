...
import { buildDashboardPayload, computeChange, type RawPipelineResults } from "./normalize";
...
describe("buildDashboardPayload", () => {
  it("computes summary cards from revenue stats, orders, and customer data", () => {
    ...
    expect(payload.summaryCards.ordersFulfilled).toEqual({ value: 12, changePercentage: 33.3, trend: "up" });
  ...
  it("derives salesOverTime, aovOverTime, and salesBreakdown from revenue stats intervals/totals", () => {
    ...
    expect(payload.charts.aovOverTime).toEqual([{ date: "Aug 1", currentPeriod: 90 / 4, previousPeriod: 90 / 3 }]);
